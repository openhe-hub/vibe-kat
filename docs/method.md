...
### Step 3: 在 784 个 patch features 上跑 K-means 聚类

K（keypoint 数量）是预设超参数，比如 K=20。K-means 在 384 维特征空间里找 K 个 cluster center，把 784 个 features 分成 K 组。

**这一步没有任何"任务感知"**——K-means 只是在做 unsupervised 的 feature space 聚类。它的隐含假设是"语义上相似的图像区域在 DINO feature 空间里也聚在一起"，这是 DINO self-supervised training 的副产品。

### Step 4: 每个 cluster 选一个代表 patch 的 2D 位置

对每个 cluster `k`，找出"最像 cluster center 的那个 patch"，记下它在 28×28 grid 上的位置 `(row, col)`。然后转回原图像素坐标（patch 中心）：
...
**2. Keypoint 索引必须跨 demo 一致**

LLM 假设 demo 1 的 keypoint_0 和 demo 2 的 keypoint_0 指代同一个语义部位。但 K-means 的 cluster 编号本身是任意的——同一个杯子在两张图里可能被分到 cluster 3 或 cluster 7。所以 KAT 在 K-means 之外**还需要一个跨图 cluster matching 的步骤**，用 cluster center 的相似度把不同图的 cluster 对齐，强制 keypoint_0 在所有图里语义一致。这一步是 KAT 最脆弱的地方——unstaged 场景下 K-means 划分本身就不稳定，cluster matching 自然也不稳。

**3. K 是固定的**

不管场景里有几个物体，输出永远是 K 个 keypoint。这是为了让 LLM prompt 的格式固定。这个约束也是 KAT 选 K-means 而不是 BBNN（best buddies nearest neighbour）的原因——BBNN 的输出数量是变长的，没法直接塞进固定格式的 prompt。
...
3. **固定 K 个点**：必须用聚类方法（K-means），不能用 BBNN（变长输出）
4. **K-means 跨图不稳**：必须额外做 cluster matching → perception 脆弱性的根源
